...
{
    private var httpClientFactory: HttpClientFactory = HttpClientFactory()

    fun setHttpClientFactory(mHttpClientFactory: HttpClientFactory) {
        this.httpClientFactory = mHttpClientFactory
    }
     ...
     override fun doInBackground(vararg arg0: Void): JSONObject?
     {
         ...
         val request = httpClientFactory.newRequest(HttpMethod.POST, mainURL)
         ...
         val text = request.send().getText() ?: return null
     ...
     override fun onPostExecute(result: JSONObject?)
     {
          ...
          try
          {
               ...
               {
                    if (!isOnline) callback!!.onValidateComplete(false, ZaloOAuthResultCode.RESULTCODE_ZALO_SDK_NO_INTERNET_ACCESS, -1, null)
                    else callback!!.onValidateComplete(false, ZaloOAuthResultCode.RESULTCODE_ZALO_UNKNOWN_ERROR, -1, null)
               }
          ...
          {
              unAuthenticate();
               callback!!.onValidateComplete(false, ZaloOAuthResultCode.RESULTCODE_UNEXPECTED_ERROR, -1, null)
          }
