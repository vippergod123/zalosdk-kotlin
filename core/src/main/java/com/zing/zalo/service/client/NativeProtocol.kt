package com.zing.zalo.service.client
...
internal object NativeProtocol
{
	...
	{
		return validateServiceIntent(context, Intent(INTENT_ACTION_PLATFORM_SERVICE).setPackage(INTENT_PACKAGE_PLATFORM_SERVICE).addCategory("android.intent.category.DEFAULT"))
	}
	...
	private fun validateServiceIntent(context: Context, intent: Intent?): Intent?
	{
		...
		if (ZaloSignatureValidator.validateSignature(context, resolveInfo.serviceInfo.packageName))
		{
            return intent
        }
        return null
